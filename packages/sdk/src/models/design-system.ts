export interface DesignSystem {
  ...
  componentPatterns: Array<{ name: string; description: string; tailwindClasses: string }>;
}

export function toDesignMd(ds: DesignSystem): string {
  return `# DESIGN.md
...
export function parseDesignMd(md: string): Partial<DesignSystem> {
  const ds: Partial<DesignSystem> = {};
  const val = (label: string): string | undefined => {
  ...
  const kvPairs = (text: string): Record<string, string> => {
  ...
  if (Object.keys(radiusKv).length) ds.borderRadius = radiusKv;

  const shadowKv = kvPairs(section("Shadows"));
  if (Object.keys(shadowKv).length) ds.shadows = shadowKv;

  const cpSection = section("Component Patterns");
  const patterns: Array<{ name: string; description: string; tailwindClasses: string }> = [];
  for (const m of cpSection.matchAll(/### (.+)\n(.+)\n```\n([\s\S]*?)```/g)) {
    patterns.push({ name: m[1], description: m[2], tailwindClasses: m[3].trim() });
  }
  if (patterns.length) ds.componentPatterns = patterns;
