...
figma.ui.onmessage = async (msg: { type: string; data: any }) => {
  if (msg.type === "import-screen") {
    ...
    const widths: Record<string, number> = { MOBILE: 390, TABLET: 820, DESKTOP: 1440, AGNOSTIC: 1280 };
    const heights: Record<string, number> = { MOBILE: 844, TABLET: 1180, DESKTOP: 900, AGNOSTIC: 800 };
  ...
  if (msg.type === "import-design-system") {
    ...
    const paintStyles: Record<string, string> = {
    ...
    for (const [name, hex] of Object.entries(paintStyles)) {
      ...
      if (rgb) style.paints = [{ type: "SOLID", color: rgb }];
    }
